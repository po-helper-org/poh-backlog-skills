...
> **Обновление после подпроекта 2a (`verify`).** Документ ниже — снимок
> состояния до подпроекта и сохранён как полезная история, но три его
> утверждения реализация опровергла:
> - CLI имеет не две, а три подкоманды — `run`, `approve`, `verify`
>   (`poh_backlog/cli.py`).
> - `expected_effect` — больше не свободный текст, а один из двух режимов,
>   `finding_gone` или `trace_label` (`poh_backlog/data/catalog.yaml`,
>   `docs/design-verify.md`, раздел 6).
> - `applied.json` больше не пуст: `verify` наполняет его ключами
>   утверждённых действий, получивших вердикт `done`
>   (`poh_backlog/memory.py: write_verdicts`).
>
> Остальной текст ниже описывает состояние на момент до подпроекта и не
> обновлялся.
